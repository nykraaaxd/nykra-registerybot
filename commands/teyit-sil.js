...
if (member.id === message.author.id) return message.channel.send(embed.setDescription(`${message.author} Kendi kayıt datanı silemezsin!`)).then(nykra => nykra.delete({timeout : 5000})).then(message.react(emoji.redemoj))
...
if(!args[1]) {
    ...
    .teyitsil @Nykra/ID erkek <miktar>
    .teyitsil @Nykra/ID kadın <miktar>`)
...
if(args[1] === 'erkek') {
let miktar = args[2]
if(!miktar) return message.channel.send(embed.setDescription(`silmek istediğinz kadar sayı girin!`)).then(nykra => nykra.delete({timeout : 7000})).then(message.react(emoji.redemoj)) 
...
if(args[1] === 'kadın') {
    ...
    exports.help = {
        name : 'kayıt-sil',
        help: "kayıt-sil [nykra/ID] [erkek/kız] [miktar]",
        cooldown: 0
     }
