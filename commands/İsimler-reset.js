...
if (member.user.bot) return message.channel.send(embed.setDescription(`${message.author} Botların isim geçmişi olmaz !`)).then(nykra => nykra.delete({timeout : 5000})).then(message.react(emoji.redemoj))
if (member.id === message.author.id) return message.channel.send(embed.setDescription(`${message.author} Kendini isim geçmişini temizleyemzsin!`)).then(nykra => nykra.delete({timeout : 5000})).then(message.react(emoji.redemoj))
...
if (!isimler) return message.channel.send(embed.setDescription(`Bu kullanıcının data'sında isim bulunamadı`)).then(message.react('❤')).then(nykra => nykra.delete({timeout:10000}))
  

kdb.delete(`isimler.${member.id}`)
message.channel.send(embed.setDescription(`${member} adlı kullanıcının isim geçmişi başarıyla temizlendi !`)).then(nykra => nykra.delete({ timeout : 10000 })).then(message.react(emoji.onayemoji))
